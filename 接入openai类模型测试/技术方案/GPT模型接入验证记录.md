...
| 维度 | OpenRouter + VPN | Claude Bridge + 老张 API | Claude Adapter + 老张 API |
|------|:----------------:|:-----------------------:|:------------------------:|
| **基本对话** | ✅ | ✅ | ✅ |
| **流式对话** | ✅ | ❌ | ✅ |
| **工具调用（非流式）** | ✅ | ✅ | ✅ |
...
| **流式工具调用** | ✅ | ❌ | ✅ |
| **需要 VPN** | ✅ 是 | ❌ 否 | ❌ 否 |
| **翻译层评分** | **3.85/5**（可靠） | **不可用**（流式翻译有 Bug） | **4.0/5**（全功能通过） |
...
| 排名 | 方案 | 评分 | 适用场景 |
|------|------|------|---------|
| 1 | **Claude Adapter + 老张 API** | **4.0/5** | **新推荐（国内部署）**，无需 VPN，全功能通过 |
| 2 | **OpenRouter + 宿主机 VPN** | 3.85 | 备选方案，依赖 VPN |
| 3 | **海外 VPS + Claude Adapter + OpenAI 直连** | 4.0 | 海外部署，无区域限制 |

---

## 六、跨 Provider 模型切换验证（补充）
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `TEST_MODEL` | 模型名称 | `openai/gpt-5.5`（OpenRouter）/ `gpt-4o`（Adapter） |
| `OPENROUTER_API_KEY` | OpenRouter 密钥 | - |
| `HTTP_PROXY` / `HTTPS_PROXY` | 代理地址（用于宿主机 VPN） | - |
| `ADAPTER_BASE_URL` | 本地翻译代理地址 | `http://localhost:8080/v1`（旧脚本）/ `http://localhost:3080`（新脚本） |
