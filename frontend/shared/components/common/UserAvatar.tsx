...
export function UserAvatar({ onClick, className = '' }: UserAvatarProps) {
  ...
  // Get first letter of display name, username, or email
  const initial = user?.displayName?.[0] || user?.username?.[0] || user?.email?.[0] || 'U';
  ...
  return (
    <button
      ...
      title={user?.displayName || user?.username || user?.email || 'User'}
    >
      {initial}
    </button>
  );
