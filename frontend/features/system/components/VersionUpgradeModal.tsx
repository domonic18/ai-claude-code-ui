 ...
 */
function VersionInfo({ currentVersion, latestVersion }: { currentVersion: string; latestVersion: string }) {
  return (
 ...
 */
function ChangelogDisplay({ releaseInfo, cleanChangelog }: { releaseInfo: ReleaseInfo; cleanChangelog: (body: string) => string }) {
  return (
    <div className="space-y-3">
 ...
 */
function ModalHeader({ onClose, releaseInfo }: { onClose: () => void; releaseInfo: ReleaseInfo | null }) {
  return (
    <div className="flex items-center justify-between">
 ...
 */
function UpdateProgress({ updateOutput }: { updateOutput: string }) {
  return (
...
/**
 * Manual upgrade instructions component
 */
...
/**
 * Hook to manage version upgrade modal state and handlers
 */
function useVersionUpgradeModal(
  currentVersion: string,
  updateAvailable: boolean,
  latestVersion: string,
  releaseInfo: ReleaseInfo | null
...
export function VersionUpgradeModal({
  ...
  const {
    ...
    handleUpdateNow,
    handleCopyCommand
  } = useVersionUpgradeModal(currentVersion, updateAvailable, latestVersion, releaseInfo);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-2xl mx-4 p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <ModalHeader onClose={onClose} releaseInfo={releaseInfo} />

        <VersionInfo currentVersion={currentVersion} latestVersion={latestVersion} />

        {releaseInfo?.body && (
          <ChangelogDisplay releaseInfo={releaseInfo} cleanChangelog={cleanChangelog} />
        )}

        {updateOutput && <UpdateProgress updateOutput={updateOutput} />}

        {!isUpdating && !updateOutput && <ManualUpgrade />}
