...
> 编写或修改代码时，必须对照以下维度自检。详细标准见 `.claude/skillls/code-review/builtin/`，评分配置见 `scoring.yaml`。

| 维度 | 权重 | 关键检查项 |
|------|------|-----------|
| **安全** | 30% | 参数化查询防 SQL 注入；前端转义防 XSS；禁止用户输入传入 exec/spawn；校验路径穿越；禁止硬编码密钥/token；日志不打印敏感数据；错误不暴露内部实现；API 端点必须认证；防止权限提升和 CSRF |
| **逻辑** | 25% | 防止 null 解引用和类型假设错误；共享状态加锁防竞态；异步不遗漏 await；禁止空 catch 和过宽异常捕获；资源在 finally 中释放；连接/句柄正确关闭；注意 off-by-one 和除零 |
| **性能** | 15% | 避免 N+1 查询；WHERE/JOIN 字段有索引；大表分页；大文件流式处理；避免循环内重复创建对象；外部 API 设超时 |
| **风格** | 15% | 命名表达意图、无缩写；函数 ≤50 行、单一职责；禁止复制粘贴、提取公共函数；魔法数字提常量；复杂逻辑注释"为什么" |
| **API** | 15% | RESTful 规范（名词复数、HTTP 方法语义正确、嵌套 ≤2 层）；参数类型/范围/必填校验；统一错误响应格式；列表返回分页信息；配置 CORS 和速率限制 |

## 5. 任务完成后协议

完成任何编码任务后，遵循此检查清单：

1. **代码审查自检**：按第 4 节维度逐一检查，修正所有 critical 和 warning 级别问题
2. **代码质量**：检查未使用的依赖/导入，验证错误处理完善，无硬编码魔法数字
3. **功能验证**：确保不破坏现有功能，WebSocket 消息流正常，文件操作安全
4. **文档更新**：新功能更新相关文档和架构调研文档（如有必要）
